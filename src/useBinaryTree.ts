...
const X_OFFSET = 180;
const Y_OFFSET = 80;
const TREE_STORAGE_KEY = 'binary-tree-simulator-tree';
...
export function useBinaryTree() {
  ...
  const [outputMessage, setOutputMessage] = useState('Output: Ready');
  ...
  const getComparisonReasoning = (target: number, nodeValue: number, context: string) =>
    `${context}: ${target} ${target < nodeValue ? '<' : '>'} ${nodeValue}, so move to the ${target < nodeValue ? 'LEFT' : 'RIGHT'} subtree.`;

  const containsValue = (node: TreeNode | null, value: number): boolean => {
  ...
  const collectInorderValues = (node: TreeNode | null, values: number[] = []): number[] => {
  ...
  useEffect(() => {
    try {
      ...
      const parsed = JSON.parse(saved) as TreeNode | null;
      if (!parsed) return;

      const positionedTree = updatePositions(parsed, INITIAL_X, INITIAL_Y, X_OFFSET);
      treeRef.current = positionedTree;
      setRoot(positionedTree);
      const restoredValues = collectInorderValues(positionedTree);
      setOutputMessage(
        restoredValues.length
          ? `Restored tree: ${restoredValues.join(', ')}`
          : 'Output: Ready'
      );
    } catch {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(TREE_STORAGE_KEY, JSON.stringify(root));
  }, [root]);
  ...
  const insertNode = async (value: number) => {
    ...
    const animateInsert = (node: TreeNode | null, val: number): TreeNode => {
      ...
      if (val < node.value) {
        newSteps.push({
          ...
          reasoning: getComparisonReasoning(val, node.value, 'Insert decision'),
          treeState: deepCopy(currentTree),
    ...
    setOutputMessage(inserted ? `Inserted: ${value}` : `Value ${value} already exists`);
  };
  ...
  const findNode = async (value: number) => {
    ...
    const animateFind = (node: TreeNode | null, val: number): boolean => {
      ...
      if (val < node.value) {
        newSteps.push({
          ...
          reasoning: getComparisonReasoning(val, node.value, 'Find decision'),
          treeState: deepCopy(currentTree),
          highlightedNodes: [node.id],
          visitedNodes: [],
        });
    ...
    const found = animateFind(currentTree, value);
    setSteps(newSteps);
    await playSteps(newSteps, treeRef.current);
    setOutputMessage(found ? `Find ${value}: Found` : `Find ${value}: Not found`);
  };
  ...
  const deleteNode = async (value: number) => {
    ...
    const animateDelete = (node: TreeNode | null, val: number): TreeNode | null => {
      ...
      if (val < node.value) {
        newSteps.push({
          ...
          codeLine: 2,
          description: `${val} < ${node.value}, searching left`,
          reasoning: getComparisonReasoning(val, node.value, 'Delete decision'),
          treeState: deepCopy(currentTree),
          highlightedNodes: [node.id],
          visitedNodes: [],
        });
    ...
    setSteps(newSteps);
    await playSteps(newSteps, positionedTree);
    setOutputMessage(deleted ? `Deleted: ${value}` : `Delete ${value}: Not found`);
  };
  ...
  const traverse = async (type: 'inorder' | 'preorder' | 'postorder') => {
    ...
    setOutputMessage(`${label}: ${traversalValues.join(', ') || 'empty tree'}`);
  };
  ...
  return {
    ...
    resetTree: () => {
      ...
      setCurrentStepIndex(-1);
      setCurrentOp('none');
      setOutputMessage('Tree reset');
      localStorage.removeItem(TREE_STORAGE_KEY);
    },
    insertNode,
    insertNodes: async (values: number[]) => {
      if (isAnimating || values.length === 0) return;

      const insertedValues: number[] = [];
      const skippedValues: number[] = [];

      for (const value of values) {
        const existsBefore = containsValue(treeRef.current, value);
      ...
      if (insertedValues.length && skippedValues.length) {
        setOutputMessage(`Inserted: ${insertedValues.join(', ')} | Skipped duplicates: ${skippedValues.join(', ')}`);
      } else if (insertedValues.length) {
        setOutputMessage(`Inserted: ${insertedValues.join(', ')}`);
      } else {
        setOutputMessage(`No new insertions. Duplicates: ${skippedValues.join(', ')}`);
      }
    },
    deleteNode,
    findNode,
    ...
    loadPreset: async (values: number[]) => {
      ...
      setIsAnimating(false);
      setCurrentOp('none');
      setOutputMessage(`Preset loaded: ${values.join(', ')}`);
    },
  };
}
